...
function setupActionButtonsListeners() {
  document.addEventListener("click", (e) => {
    const addToCartBtn = e.target.closest(
      // 'button.productcomp__item:not(.productcomp__item--favorite):not(.productcomp__item--view)'
      "button.productcomp__item:not(.productcomp__item--view)"
    );
    if (addToCartBtn) {
      addToCartBtn.classList.toggle("productcomp__item--active");
    }
  });
}
